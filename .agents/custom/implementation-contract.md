...
2. **If no existing branch:** create a new branch (`feat/<slug>`, `fix/<slug>`, `design/<slug>`, `refactor/<slug>`, `docs/<slug>`, `chore/<slug>`, or `test/<slug>` from `master`),
...
```
gh pr comment <n> --body "✅ Complete: <summary>"           # if done (PR exists)
gh pr comment <n> --body "⏸ Paused at turn limit: completed <X>, remaining: <Y>"  # if not done (PR exists)
gh issue comment <ISSUE NUMBER> --body "✅ Complete: <summary>"           # if done (no PR yet)
gh issue comment <ISSUE NUMBER> --body "⏸ Paused at turn limit: completed <X>, remaining: <Y>"  # if not done (no PR yet)
```
