...
describe("file validation", () => {
  ...
  it("returns 413 when file exceeds 100 MB", async () => {
    // Create a File whose .size is > 100 MB without allocating the bytes
    const bigFile = { size: 101 * 1024 * 1024, name: "big.zip", type: "application/zip", arrayBuffer: async () => new ArrayBuffer(0) } as unknown as File;
    const formData = new FormData();
