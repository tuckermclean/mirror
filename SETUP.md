...
> **README discrepancy:** `README.md` currently lists "Node.js 18+ / pnpm 8+".
> That line is stale. The `engines` field in `package.json` (Node `>=20`,
> pnpm `>=10`) is the source of truth — use those.
...
|---|---|
| `pnpm test:unit`, `pnpm typecheck`, `pnpm lint`, `pnpm infra:test` | None — placeholders are fine (no DB, no network) |
...
|---|---|---|
| `DATABASE_URL` | Yes | Local: `postgres://mirror:mirror@localhost:5432/mirror` (from `.env.example`). Compose injects the in-network form automatically. |
| `REDIS_URL` | Yes | Local: `redis://localhost:6379`. Compose injects `redis://redis:6379`. |
| `COOKIE_ENCRYPTION_KEY` | Yes | 32-byte base64. Generate: `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"` (equivalent to DEPLOY.md's `openssl rand -base64 32`). |
...
```bash
docker compose --profile lite up -d --wait   # postgres + redis (+ web)
...
```bash
pnpm db:push        # Dev: push the Drizzle schema directly (no migration files)
...
provides pgvector; run `pnpm db:push` against it before `pnpm test:integration`
