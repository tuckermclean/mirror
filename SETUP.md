...
```bash
gh secret set CLERK_PUBLISHABLE_KEY               # pk_test_... (Clerk public/publishable client key — safe to expose)
...
| Generation returns HTTP 402 `monthly_cap_reached` | Monthly spend hit `LLM_MONTHLY_CAP_USD` (default `$20`) | Raise `LLM_MONTHLY_CAP_USD` in `.env.local` (local dev only), or wait for `resets_at` — the LLM spend ledger resets at the month boundary, so a month rollover clears the cap without changing the limit. |
