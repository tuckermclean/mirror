...
**One-click delete everything:** The `/settings/delete-account` route triggers a Postgres transaction that cascades deletes across all tables in dependency order, then calls `clerk.users.deleteUser(clerkId)`. R2 objects are deleted via the Cloudflare R2 API in the same Inngest function (non-transactional but retried). The function returns to the client only after all deletions complete. Test coverage in `tests/e2e/auth.spec.ts`.
