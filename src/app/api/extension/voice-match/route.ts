 ...
 * Returns the *trimmed* text so downstream never receives surrounding (or
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // `profileText` is already trimmed by parseProfileText (null if empty after
  // trim), so the length check below operates on the same value the scorer
  // receives — whitespace-only input is rejected as required, not measured raw.
