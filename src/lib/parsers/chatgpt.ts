...
function conversationToMessages(conv: ChatGPTConversation): ParsedMessage[] {
  ...
  // Walk the tree in insertion order (children ordering)
    ...
    if (msg && isTextRole(msg.author.role) && msg.content) {
      ...
      if (text) {
        messages.push({
          ...
          content: text,
          timestamp: msg.create_time != null ? new Date(msg.create_time * 1000).toISOString() : undefined,
 ...
 * Throws if the zip is invalid or contains no conversations.
 */
export async function parseChatGPTExport(data: Uint8Array): Promise<ParsedChatHistory> {
  if (data.length === 0) {
    throw new Error("Empty zip data — cannot parse ChatGPT export");
  }
